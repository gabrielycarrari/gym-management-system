...
public class AnchorPaneTreinoDialogController implements Initializable {
    @FXML
    ...
    private void handleButtonConfirmar() {
        ...
        if(validate()){
            ...
            treino.setDataInicio(datePickerDataInicio.getValue());
            treino.setDataFinal(datePickerDataFinal.getValue());
            ...
            try {
                ...
                Treino idTreinoInserido = treinoDAO.buscarUltimoTreino();
                Boolean treinoValido = treinoDAO.findByPeriodo(idTreinoInserido.getIdTreino(), treino);
                
                if(treinoValido){
                    for(int i = 0; i < listExerciciosDoTreino.size(); i++){
                        TreinoExercicio treinoExercicio = new TreinoExercicio();
                        treinoExercicio.setIdTreino(idTreinoInserido.getIdTreino());
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR, "Não foi possível salvar o treino, já existe um treino neste periodo!", null);
                    alert.show();
                }
